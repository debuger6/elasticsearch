...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    static boolean executeBulkItemRequest(BulkPrimaryExecutionContext context, UpdateHelper updateHelper, LongSupplier nowInMillisSupplier,
        ...
        if (context.getPrimary().indexSettings().isGetCacheEnabled() && result.getFailure() == null) {
            if (isDelete) {
                IndicesService.GetCache.invalidate(id);
                long end = System.nanoTime();
                logger.debug("GetCache.invalidate cost: " + (end-start)/1000 + "us");
            } else {
                IndicesService.GetCache.put(id, source);
                long end = System.nanoTime();
                logger.debug("GetCache.put cost: " + (end-start)/1000 + "us");
            }
        }
